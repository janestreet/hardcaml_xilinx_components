...
let rec expr ?(id = Fn.id) : Vhdl.Expression.t -> string = function
  ...
  | Op (op, a, b) ->
    String.concat
      ["("
      ; (String.concat ~sep:" " [ expr ~id a; op; expr ~id b ])
      ; ")"]
  | Neg a        -> "-" ^ expr a
  | Others c     -> "(others => '" ^ c ^ "')"
  | X01 s        -> "TO_X01(" ^ s ^ ")"
...
module Inferred_port = struct
  ...
  let std_logic port =
    ...
    in
    let fail () = raise_s [%message "invalid port_std_logic value" (port : Vhdl.Port.t)] in
  ...
  let generic_vector port (range : Vhdl.Range.t option) ~is_bitvector =
    ...
    let create =
      if is_bitvector
      then create_bit_vector
      else create_std_logic_vector
    in
      ...
      | Some expr ->
        let rec f : Vhdl.Expression.t -> string option = function
          ...
          | Hex s ->
            ...
            Some (Hardcaml.Constant.of_hex_string ~signedness:Signed ~width s
                  |> Hardcaml.Constant.to_binary_string)
          | Op ("&", a, b) -> Option.map2 (f a) (f b) ~f:(^)
          | _ -> None
        in
        Option.map (f expr) ~f:(fun s ->
          "\"" ^ s ^ "\"", String.length s)
    in
  ...
  let default_types =
    lines (
      List.map generics ~f:(fun param ->
        sprintf "    val %s : %s"
          param.ocaml_name param.default_type))
  in
  let default_values =
    lines (
      List.map generics ~f:(fun param ->
        sprintf "    let %s = %s"
          param.ocaml_name param.default_value))
  in
  let ocaml_params =
    lines (
      List.map generics ~f:(fun param ->
        sprintf "      Hardcaml.Parameter.create ~name:\"%s\" ~value:(%s P.%s);"
          param.port.name param.constr param.ocaml_name))
  in
  let header =
    String.concat [ "\
...
" ] in
  let interface mod_name dir =
    let ports =
      List.filter comp.ports ~f:(fun port -> Vhdl.Port_direction.equal port.dir dir) in
      ...
      let ocaml_ports =
        lines (
          List.map ports ~f:(fun port ->
            sprintf "        %s : 'a[@bits %s][@rtlname \"%s\"];"
              port.ocaml_name port.bits port.port.name))
      in
      String.concat ["
...
"]
  ...
  String.concat
    ...
    let create ?lib ?arch ?instance inputs =
      create ?lib ?arch ?instance ~parameters:params ~name:\"";comp.name;"\" inputs
  end
end
